...
export async function POST(request: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const candidate = await prisma.candidate.findFirst({ where: { userId: user.id }, select: { id: true, resumeUrl: true } });
    if (!candidate?.resumeUrl) return NextResponse.json({ message: "Upload a resume before applying." }, { status: 400 });
