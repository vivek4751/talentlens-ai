...
function CandidateProfile({ candidateMode = false }: { candidateMode?: boolean }) {
  const [, setLocation] = useLocation();
  return <div className="page-wrap"><button className="back-link" onClick={() => setLocation(candidateMode ? "/jobs" : "/candidates")}>← {candidateMode ? "Open roles" : "All candidates"}</button><section className="candidate-hero"><div className="candidate-identity"><span className="candidate-monogram">MP</span><div><p className="section-index">CANDIDATE / 00084</p><h1>{candidateMode ? "Your profile" : "Maia Patel"}</h1><p>Senior Product Designer · Bengaluru, India</p><div className="hero-contact"><span><Mail size={14} />maia.patel@example.com</span><span><MapPin size={14} />Hybrid-ready</span><span><Linkedin size={14} />Verified profile</span></div></div></div><div className="candidate-score-hero"><p className="micro-label">BEST MATCH</p><strong>92.0</strong><span>Senior Product Designer</span></div></section>
